...
class Terminal {
    ...
    static terminalService() {
        if (!this._terminalService) {
            if (process.platform === 'win32') this._terminalService = new WindowsTerminalService();else if (process.platform === 'darwin') this._terminalService = new MacTerminalService();else if (process.platform === 'linux') this._terminalService = new LinuxTerminalService();else {
...
class LinuxTerminalService extends DefaultTerminalService {
    launchInTerminal(dir, args, envVars) {
        return new Promise((resolve, reject) => {
            ...
            const termArgs = ['--title', `"${ LinuxTerminalService.TERMINAL_TITLE }"`, '-x', 'bash', '-c', `\'\'${ bashCommand }\'\'` // wrapping argument in two sets of ' because node is so "friendly" that it removes one set...
            ];
            // merge environment variables into a copy of the process.env
            const env = extendObject(extendObject({}, process.env), envVars);
            const options = {
...
class MacTerminalService extends DefaultTerminalService {
    launchInTerminal(dir, args, envVars) {
        return new Promise((resolve, reject) => {
            ...
            const osaArgs = [Path.join(__dirname, './terminalHelper.scpt'), '-t', MacTerminalService.TERMINAL_TITLE, '-w', dir];
            for (const a of args) {
                osaArgs.push('-pa');
                osaArgs.push(a);
            }
            ...
            osa.on('exit', code => {
                if (code === 0) {
                    resolve(); // since cmd is not the terminal process but just the osa tool, we do not pass it in the resolve to the caller
                        ...
                        if (stderr) reject(new Error(stderr));else reject(new Error('exit code: ' + code));
                    }
            });
        });
    }
