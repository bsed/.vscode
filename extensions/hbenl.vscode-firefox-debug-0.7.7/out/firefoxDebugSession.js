...
var FirefoxDebugSession = (function (_super) {
    ...
    FirefoxDebugSession.prototype.convertFirefoxUrlToPath = function (url) {
        ...
        for (var i = 0; i < this.pathMappings.length; i++) {
            ...
            if (typeof from === 'string') {
                ...
                var match = from.exec(url);
                if (match) {
                    var path_2 = to + match[1];
                    if (this.isWindowsPlatform) {
                        path_2 = path_2.replace(/\//g, '\\');
                    }
                    pathConversionLog.debug("Converted url " + url + " to path " + path_2);
                    return path_2;
                }
            }
        }
    ...
    FirefoxDebugSession.prototype.readCommonConfiguration = function (args) {
        ...
        if (args.addonType) {
            ...
            if (this.addonType === 'addonSdk') {
                var rewrittenAddonId = addonIdOrErrorMsg.replace("@", "-at-");
                var sanitizedAddonPath = this.addonPath;
                ...
                var rewrittenAddonId = this.addonId.replace('{', '%7B').replace('}', '%7D');
                var sanitizedAddonPath = this.addonPath;
                if (sanitizedAddonPath[sanitizedAddonPath.length - 1] === '/') {
                    sanitizedAddonPath = sanitizedAddonPath.substr(0, sanitizedAddonPath.length - 1);
                }
    ...
    FirefoxDebugSession.prototype.startSession = function (socket) {
        ...
        socket.on('close', function () {
            ...
            _this.firefoxDebugSocketClosed = true;
    ...
    FirefoxDebugSession.prototype.evaluateRequest = function (response, args) {
        ...
        if (args.frameId) {
            ...
            for (var i = 1; i < this.nextThreadId; i++) {
