...
function makeDiagnostic(problem) {
    var message = (problem.ruleId != null)
        ...
        : "" + problem.message;
    var startLine = problem.line - 1;
    var startChar = problem.column - 1;
    var endLine = problem.endLine ? problem.endLine - 1 : startLine;
    var endChar = problem.endColumn ? problem.endColumn - 1 : startChar;
    return {
        message: message,
...
connection.onInitialize(function (params) {
    ...
    var noEslintLib = new vscode_languageserver_1.ResponseError(100, 'Failed to load eslint library.', { retry: false });
    var noCLIEngine = new vscode_languageserver_1.ResponseError(101, 'The eslint library doesn\'t export a CLIEngine. You need at least eslint@1.0.0', { retry: false });
    ...
    if (legacyModuleResolve) {
        ...
        return vscode_languageserver_1.Files.resolveModule2(rootPath, 'eslint', nodePath, trace).then(function (value) {
            if (!value.CLIEngine) {
                return noCLIEngine;
            }
            lib = value;
