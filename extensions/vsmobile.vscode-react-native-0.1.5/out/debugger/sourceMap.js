...
var SourceMapUtil = (function () {
     ...
     */
    SourceMapUtil.prototype.getSourceMapURL = function (scriptUrl, scriptBody) {
        var result = null;
        // scriptUrl = "http://localhost:8081/index.ios.bundle?platform=ios&dev=true"
        var sourceMappingRelativeUrl = this.getSourceMapRelativeUrl(scriptBody); // sourceMappingRelativeUrl = "/index.ios.map?platform=ios&dev=true"
        if (sourceMappingRelativeUrl) {
            var sourceMappingUrl = url.parse(sourceMappingRelativeUrl);
     ...
     *
     */
    SourceMapUtil.prototype.updateSourceMapFile = function (sourceMapBody, scriptPath, sourcesRootPath) {
        var _this = this;
        try {
            var sourceMap = JSON.parse(sourceMapBody);
            sourceMap.sources = sourceMap.sources.map(function (sourcePath) {
                return _this.updateSourceMapPath(sourcePath, sourcesRootPath);
            });
     ...
     */
    SourceMapUtil.prototype.updateScriptPaths = function (scriptBody, sourceMappingUrl) {
     ...
     */
    SourceMapUtil.prototype.updateSourceMapPath = function (sourcePath, sourcesRootPath) {
        var relativeSourcePath = path.relative(sourcesRootPath, sourcePath);
     ...
     */
    SourceMapUtil.prototype.makeUnixStylePath = function (p) {
        var pathArgs = p.split(path.sep);
     ...
     */
    SourceMapUtil.prototype.getSourceMapRelativeUrl = function (body) {
        var match = body.match(SourceMapUtil.SourceMapURLRegex);
        // If match is null, the body doesn't contain the source map
        return match ? match[2] : null;
    };
    SourceMapUtil.SourceMapURLRegex = /\/\/(#|@) sourceMappingURL=(.+?)\s*$/m;
    return SourceMapUtil;
}());
