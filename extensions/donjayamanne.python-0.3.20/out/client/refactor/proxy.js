...
var RefactorProxy = (function (_super) {
    ...
    RefactorProxy.prototype.extractVariable = function (document, name, filePath, range) {
        var command = "{\"lookup\":\"extract_variable\", \"file\":\"" + filePath + "\", \"start\":\"" + document.offsetAt(range.start) + "\", \"end\":\"" + document.offsetAt(range.end) + "\", \"id\":\"1\", \"name\":\"" + name + "\"}";
        return this.sendCommand(command, telemetryContracts_1.REFACTOR.ExtractVariable);
    };
    RefactorProxy.prototype.extractMethod = function (document, name, filePath, range) {
        var command = "{\"lookup\":\"extract_method\", \"file\":\"" + filePath + "\", \"start\":\"" + document.offsetAt(range.start) + "\", \"end\":\"" + document.offsetAt(range.end) + "\", \"id\":\"1\",\"name\":\"" + name + "\"}";
        return this.sendCommand(command, telemetryContracts_1.REFACTOR.ExtractVariable);
    };
