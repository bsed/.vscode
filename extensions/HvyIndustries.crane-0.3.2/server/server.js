...
connection.onDefinition((position, cancellationToken) => {
    return new Promise((resolve, reject) => {
        let path = Files_1.Files.getPathFromUri(position.textDocument.uri);
        let filenode = getFileNodeFromPath(path);
        let definitionProvider = new definition_1.DefinitionProvider(position, path, filenode, workspaceTree);
