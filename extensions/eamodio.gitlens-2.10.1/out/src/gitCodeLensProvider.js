...
class GitCodeLensProvider {
    ...
    provideCodeLenses(document, token) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (languageLocations.location === configuration_1.CodeLensLocation.None)
                return lenses;
            const gitUri = yield gitProvider_1.GitUri.fromUri(document.uri, this.git);
    ...
    _resolveGitRecentChangeCodeLens(lens, token) {
        ...
        if (this._config.advanced.debug && this._config.advanced.output.level === logger_1.OutputLevel.Verbose) {
            title += ` [Commit (${recentCommit.sha}), Symbol (${vscode_1.SymbolKind[lens.symbolKind]}), Lines (${lens.blameRange.start.line + 1}-${lens.blameRange.end.line + 1})]`;
        }
        switch (this._config.codeLens.recentChange.command) {
            ...
            case configuration_1.CodeLensCommand.ShowQuickRepoHistory: return this._applyShowQuickRepoHistoryCommand(title, lens, blame, recentCommit);
            default: return lens;
        }
    }
    _resolveGitAuthorsCodeLens(lens, token) {
        ...
        if (this._config.advanced.debug && this._config.advanced.output.level === logger_1.OutputLevel.Verbose) {
            title += ` [Authors (${system_1.Iterables.join(system_1.Iterables.map(blame.authors.values(), _ => _.name), ', ')}), Symbol (${vscode_1.SymbolKind[lens.symbolKind]}), Lines (${lens.blameRange.start.line + 1}-${lens.blameRange.end.line + 1})]`;
        }
        switch (this._config.codeLens.authors.command) {
            ...
            case configuration_1.CodeLensCommand.ShowQuickRepoHistory: return this._applyShowQuickRepoHistoryCommand(title, lens, blame);
            default: return lens;
        }
    }
    ...
    _applyShowQuickCommitDetailsCommand(title, lens, blame, commit) {
        lens.command = {
            ...
            arguments: [vscode_1.Uri.file(lens.uri.fsPath), undefined, commit]
        };
        return lens;
    }
