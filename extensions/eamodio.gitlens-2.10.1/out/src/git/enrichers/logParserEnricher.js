    ...
    enrich(data, type, fileNameOrRepoPath, isRepoPath = false) {
        ...
        for (let i = 0, len = entries.length; i < len; i++) {
            ...
            if (!commit) {
                ...
                commit = new git_1.GitLogCommit(repoPath, entry.sha, relativeFileName, entry.author, moment(entry.authorDate).toDate(), entry.summary, entry.status, entry.fileStatuses, undefined, entry.originalFileName);
