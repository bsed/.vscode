...
class Formatter {
    ...
    formatDocument(document) {
        return new Promise((resolve, reject) => {
            ...
            let formatCommandBinPath = goPath_1.getBinPath(this.formatCommand);
            ...
            cp.execFile(formatCommandBinPath, [...formatFlags, filename], {}, (err, stdout, stderr) => {
                try {
                    if (err && err.code === 'ENOENT') {
                        goInstallTools_1.promptForMissingTool(this.formatCommand);
                        return resolve(null);
                    }
                    if (err) {
                        console.log(err);
                    ...
                    util_1.sendTelemetryEvent('format', { tool: this.formatCommand }, { timeTaken });
