...
suite('Go Extension Tests', () => {
    ...
    function testDefinitionProvider(tool) {
        let provider = new goDeclaration_1.GoDefinitionProvider(tool);
    ...
    function testSignatureHelpProvider(tool, testCases) {
        let provider = new goSignature_1.GoSignatureHelpProvider(tool);
    ...
    function testHoverProvider(tool, testCases) {
        let provider = new goExtraInfo_1.GoHoverProvider(tool);
        let uri = vscode.Uri.file(path.join(fixturePath, 'gogetdocTestData', 'test.go'));
        return vscode.workspace.openTextDocument(uri).then((textDocument) => {
    ...
    test('Replace vendor packages with relative path', (done) => {
        ...
        vendorSupportPromise.then((vendorSupport) => {
            let gopkgsPromise = new Promise((resolve, reject) => {
                cp.execFile(goPath_1.getBinPath('gopkgs'), [], (err, stdout, stderr) => {
