...
function generateSpec(target, options) {
...
function generateSpecForChai(target, options) {
    var q = options.quote;
    var s = options.semicolon ? ';' : '';
    ...
    if (target instanceof Array) {
        ...
        Object.keys(target).map(function (key) {
            ...
            var keyRef = /^[a-zA-Z0-9_]+$/g.test(key) ? "." + key : "[" + q + key + q + "]";
            var variableName = "" + options.variableName + keyRef;
