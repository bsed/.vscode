...
var cInfo = require("./configinfo");
var cKeys = cInfo.keys;
...
function activate(context) {
    ...
    var disposable = vscode.commands.registerCommand(cInfo.command("search"), function () {
        ...
        // Get config settings
        var config = vscode.workspace.getConfiguration(cInfo.group);
        var useDefaultOnly = config.get(cKeys.useDefaultProviderOnly);
        var useDefaultForSelection = config.get(cKeys.alwaysUseDefaultForSelection);
        var skipInputForSelection = config.get(cKeys.noInputBoxIfTextSelected);
        if (!utils.isNullOrEmpty(selectedText) && skipInputForSelection) {
            ...
            vscode.window.showInputBox(options).then(function (q) {
                return searchFor(q, (useDefaultOnly || utils.startsWith(q, selectedText)));
            });
        }
    });
...
function getSearchUrl(query, useDefault) {
    if (useDefault === void 0) { useDefault = false; }
    // Get config stuff
    var config = vscode.workspace.getConfiguration(cInfo.group);
    var searchProviders = config.get(cKeys.searchProviders);
    var defaultProvider = config.get(cKeys.defaultProvider);
    var providerID = query.split(" ", 1)[0];
    // Backwards compatibility with old config format
    var oldSearchProvider = config.get(cInfo.depricatedKeys.searchprovider);
...
function searchFor(query, useDefault) {
    if (useDefault === void 0) { useDefault = false; }
    if (!query) {
        return;
    }
