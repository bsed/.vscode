...
function getActionFromShebang() {
    ...
    if (firstLine.match(/^#!\s*(.*)\s*$/)) {
        ...
        for (var key in shebangMap) {
            try {
                if (new RegExp(key).test(firstLine))
                    return shebangMap[key];
            }
            catch (e) { }
        }
        return cmd;
...
function activate(ctx) {
    var currentProcess;
    ctx.subscriptions.push(vscode.commands.registerCommand('extension.runner.stop', function () {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('extension.runner.start', function () {
