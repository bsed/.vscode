...
function check(filename, goConfig) {
    ...
    if (!!goConfig['buildOnSave']) {
        ...
        runningToolsPromises.push(runTool(goPath_1.getGoRuntimePath(), args, cwd, 'error', true, null, 'No "go" binary could be found in GOROOT: ' + process.env['GOROOT'] + '"'));
    }
    ...
    if (!!goConfig['vetOnSave']) {
        ...
        runningToolsPromises.push(runTool(goPath_1.getGoRuntimePath(), ['tool', 'vet'].concat(vetFlags, [filename]), cwd, 'warning', true, null, 'No "go" binary could be found in GOROOT: "' + process.env['GOROOT'] + '"'));
    }
