...
function goTest(config) {
    return new Promise(function (resolve, reject) {
        ...
        goStatus_1.outputChannel.show(2);
        ...
        proc.stdout.on('data', function (chunk) { return goStatus_1.outputChannel.append(chunk.toString()); });
        proc.stderr.on('data', function (chunk) { return goStatus_1.outputChannel.append(chunk.toString()); });
        proc.on('close', function (code) {
            if (code) {
                goStatus_1.outputChannel.append('Error: Tests failed.');
            }
            else {
                goStatus_1.outputChannel.append('Success: Tests passed.');
            }
            resolve(code === 0);
        });
    });
