...
function _publish(packagePath, pat, manifest) {
    ...
    return readFile(packagePath, 'base64').then(function (extensionManifest) {
        var fullName = manifest.publisher + "." + manifest.name + "@" + manifest.version;
        ...
        return api.getExtension(manifest.publisher, manifest.name, null, GalleryInterfaces_1.ExtensionQueryFlags.IncludeVersions)
            ...
            var promise = extension
                ? api.updateExtension({ extensionManifest: extensionManifest }, manifest.publisher, manifest.name)
                : api.createExtension({ extensionManifest: extensionManifest });
            return promise
...
function versionBump(cwd, version) {
    ...
    return package_1.readManifest(cwd)
        ...
        switch (version) {
            ...
            case 'minor':
            case 'patch':
                return { manifest: manifest, version: semver.inc(manifest.version, version) };
            default:
                var updatedVersion = semver.valid(version);
                if (!updatedVersion) {
                    return Promise.reject("Invalid version " + version);
                }
                return { manifest: manifest, version: updatedVersion };
        }
    }).then(function (_a) {
        var manifest = _a.manifest, version = _a.version;
        if (version !== manifest.version) {
            manifest.version = version;
            return package_1.writeManifest(cwd, manifest);
        }
    });
}
function publish(options) {
    ...
    if (options.packagePath) {
        ...
        promise = versionBump(options.cwd, options.version)
            ...
            .then(function (packagePath) { return package_1.pack({ packagePath: packagePath, cwd: cwd_1, baseContentUrl: baseContentUrl_1, baseImagesUrl: baseImagesUrl_1 }); });
    }
