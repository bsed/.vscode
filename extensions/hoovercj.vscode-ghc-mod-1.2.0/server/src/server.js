...
function initializeGhcMod() {
    ...
    if (ghcMod) {
        ...
        logger.error('Try configuring user or workspace settings for haskell.ghcMod.executablePath', ['Workspace', 'User'], (action) => {
            connection.sendRequest(OpenSettingsRequest.type, action);
        });
    }
...
function initializeOnDefinition() {
    connection.onDefinition((documentInfo) => {
        ...
        return ghcModProvider.getDefinitionLocation(document, uriToFilePath(document.uri), documentInfo.position, workspaceRoot);
    });
}
