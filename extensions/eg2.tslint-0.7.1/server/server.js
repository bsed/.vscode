...
function recordCodeAction(document, diagnostic, problem) {
    ...
    let fix = null;
    if (problem.getFix && problem.getFix()) {
...
connection.onCodeAction((params) => {
    ...
    if (documentFixes) {
        ...
        if (result.length > 0) {
            ...
            if (same.length > 1) {
                result.push(server.Command.create(`Fix all "${same[0].problem.getRuleName()}" tslint warnings`, 'tslint.applySameFixes', uri, documentVersion, concatenateEdits(same)));
            }
            // create a command to fix all the warnings with fixes
            if (all.length > 1) {
                result.push(server.Command.create(`Fix all auto-fixable problems`, 'tslint.applyAllFixes', uri, documentVersion, concatenateEdits(all)));
            }
        }
    }
...
function createAutoFix(problem, document, fix) {
    let edits = null;
    function isTslintFix(fix) {
        return fix.replacements !== undefined;
    }
    if (isTslintFix(fix)) {
